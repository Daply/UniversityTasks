...
public class Client implements Runnable {
    ...
    public List<String> getMessages() {
        ...
        try {
            ...
            for (Object o : jsonArray) {
                ...
                int messageIndex = s.indexOf("\"message\":");
                int userIndex = s.indexOf("\"user\":");
                String messageText = "";
                while (messageIndex != userIndex - 2) {
                    messageText += s.charAt(messageIndex);
                    messageIndex++;
                }
                messageText = messageText.replaceAll("\"message\":", "");
                messageText = messageText.replaceAll("\"", "");


                String userName = "";
                while (s.charAt(userIndex) != '}') {
                    userName += s.charAt(userIndex);
                    userIndex++;
                }
                userName = userName.replaceAll("\"user\":", "");
                userName = userName.replaceAll("\"", "");
                String result = userName + ": " + messageText;
    ...
    public void run() {
        ...
        if (flag == 0) {
            ...
            username = scannerForName.nextLine();
            flag++;
        }
